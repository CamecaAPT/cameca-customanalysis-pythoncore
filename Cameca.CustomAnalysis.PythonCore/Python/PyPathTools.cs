...
internal static class PyPathTools
{
	...
	/// Assume all files in the directory match that pattern python\d+.dll
	/// </summary>
	...
	public static string? ResolvePythonDll(string directory)
	{
		...
		{
			if (Regex.Match(candidate.Name, @"python(?<Version>\d+).dll", RegexOptions.IgnoreCase) is not { Success: true } match) continue;
